<template>
  <div class="flex flex-col items-center h-full space-y-8 bg-gradient-to-br from-[rgb(28,31,50)] to-gray-900">
    <main class="w-screen xs:max-w-screen-xs m-auto h-full grid grid-rows-[auto,1fr,auto]">
      <div class="relative">
        <FLHeader @help="() => (state.modals.help = true)" @settings="() => (state.modals.settings = true)" />
        <div class="absolute w-full p-8" v-if="state.notInCorpus">
          <div class="p-4 rounded text-center w-full bg-gray-300 text-gray-900">not in word list</div>
        </div>
      </div>
      <section class="p-6 flex justify-center max-w-screen-xs w-full m-auto h-full overflow-auto" v-if="!state.loading" :class="{ 'items-center': word && state.word.length > 3 }">
        <div class="grid grid-flow-row-dense gap-2 px-8 xxs:px-2" :style="`grid-template-columns: repeat(${state.word.length}, minmax(0, 6rem));`">
  ...
  <div class="fixed top-0 right-0 text-white text-sm p-5 max-w-xs w-screen bg-gray-700" v-if="state.debugging">
    <pre><code>word: {{state.word}}</code></pre>
    <pre class="overflow-scroll h-64 w-full"><code>keyColors: {{state.keyColors}}</code></pre>
    <pre class="overflow-scroll h-64 w-full"><code>results: {{state.results}}</code></pre>
    <pre class="overflow-scroll h-64 w-full"><code>guesses: {{state.guesses}}</code></pre>
  </div>
  <div v-if="!state.loading">
    ...
    <FLModal v-model:open="state.modals.settings" title="SETTINGS">
      ...
      <label class="block text-left">
        <p>Guesses</p>
        <select v-model="state.numGuesses" class="appearance-none w-full px-2 py-1 rounded border border-gray-500 bg-transparent text-white">
...
export default {
  ...
  setup() {
    const { keys, state, keyPress, newGame } = useGame()
